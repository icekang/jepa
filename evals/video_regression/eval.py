...
def main(args_eval, resume_preempt=False):
    ...
    if resume_checkpoint:
        encoder, classifier, optimizer, scaler, start_epoch, best_val_mse = load_checkpoint(
            device=device,
            r_path=latest_path,
            encoder=encoder,
            classifier=classifier,
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        train_r2, train_mae, train_mse = run_one_epoch(
            device=device,
            training=True,
            encoder=encoder,
            classifier=classifier,
        ...
        val_r2, val_mae, val_mse = run_one_epoch(
            device=device,
            training=False,
            encoder=encoder,
            classifier=classifier,
        ...
        logger.info('[%5d] train: %.3f%% val: %.3f%%' % (epoch + 1, train_mse, val_mse))
        logger.info(f'R2: {train_r2:.5f}, MAE: {train_mae:.5f}, MSE: {train_mse:.5f}')
        logger.info(f'Validation R2: {val_r2:.5f}, MAE: {val_mae:.5f}, MSE: {val_mae:.5f}')
        if rank == 0:
        ...
        if val_mse > best_val_mse:
            logger.info(f'New best MSE: {val_mse:.5f} improved from {best_val_mse}, saving to {best_path}')
            best_val_mse = val_mse
            save_checkpoint(epoch + 1, best_path)
        else:
            logger.info(f'MSE: {val_mse:.5f} did not improve from {best_val_mse}')
    ...
    encoder, classifier, _, _, _, best_val_mse = load_checkpoint(
        device=device,
        r_path=best_path,
        encoder=encoder,
        classifier=classifier,
        opt=optimizer,
        scaler=scaler)
    logger.info(f'Loaded best model with MSE: {best_val_mse:.5f}')
    test_r2, test_mae, test_mse = run_one_epoch(
            device=device,
            training=False,
            encoder=encoder,
            classifier=classifier,
            ...
            freeze_encoder=freeze_encoder,
            config=args_eval)
    logger.info(f'Test R2: {test_r2:.5f}, MAE: {test_mae:.5f}, MSE: {test_mse:.5f}')
...
def load_checkpoint(
    ...
    try:
        ...
        if 'best_val_mse' in checkpoint:
            ...
            logger.info(f'loaded best validation dice from epoch {epoch} with mse: {best_val_mse}')
    ...
    return encoder, classifier, opt, scaler, epoch, best_val_mse
